...
function Tdets() {
  ...
  return info ? (
    <div style={{
      ...
      {info.videos &&
          <Link to={`${pathname}/trailer`} className="icon h-14 w-14 bg-[#6556CD] absolute top-[25%] left-[10%] text-white flex items-center justify-center rounded-full text-2xl ">
      ...
      <div className="w-full p-10 flex gap-10">
        ...
        <div className="content">
          <h1 className='text-5xl font-semibold text-white'>{info.detail.name || info.detail.original_name || info.detail.original_title || info.detail.title}
            ...
            <small className='text-2xl text-zinc-300 ml-2'>({info.detail.first_air_date.split("-")[0]})</small>
          </h1>
      ...
      <h1 className='text-3xl font-semibold text-white pl-5 mb-10'>Seasons</h1>

      <Horcards data={info.detail.seasons.length > 0 && info.detail.seasons} />


      {/* <Cards data={info.credits} title="people "/> */}


      <h1 className='text-3xl font-semibold text-white pl-5 mb-10'>Recommendations & Similer</h1>
